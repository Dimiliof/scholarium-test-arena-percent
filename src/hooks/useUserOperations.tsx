...
export const useUserOperations = () => {
  ...
  const login = async (email: string, password: string, loginRecords: LoginRecord[], setLoginRecords: React.Dispatch<React.SetStateAction<LoginRecord[]>>) => {
    console.log(`Προσπάθεια σύνδεσης με: ${email}`);
    
    // Ειδική περίπτωση για τον κύριο διαχειριστή
    if (email === "liofisdimitris@gmail.com" && password === "Skatadi21!") {
      console.log("Επιτυχής σύνδεση κύριου διαχειριστή!");
      
      // Δημιουργία καταγραφής σύνδεσης
      const loginRecord: LoginRecord = {
        userId: "admin-special-id",
        userName: "Διαχειριστής Συστήματος",
        email: email,
        role: "admin",
        timestamp: Date.now(),
      };
    ...
    if (!storedUsers) {
      console.log("Δεν βρέθηκαν χρήστες στο localStorage");
      return false;
    }
    ...
    if (user) {
      console.log(`Επιτυχής σύνδεση για τον χρήστη: ${user.firstName} ${user.lastName}`);
    ...
    console.log("Αποτυχία σύνδεσης: Δεν βρέθηκε ο χρήστης ή λάθος κωδικός");
    return false;
  };
